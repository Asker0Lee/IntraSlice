...
class sobp_LlamaAttention(LlamaAttention):
...
class SoBPLlamaDecoderLayer(LlamaDecoderLayer):
    """
    ...
    def __init__(self, config, layer_idx, sparsity):
        ...
        super().__init__(config, layer_idx)
        self.self_attn = sobp_LlamaAttention(config, layer_idx)
    ...
    def sparse_model(self, sparsity):
        ...
        prune_head_num = int(round(self.self_attn.num_heads*sparsity*alpha))
     
        prune_interdim_num = int((prune_param - prune_head_num*self.per_head_param)//self.per_neuron_param)
        ...
        if self.self_attn.num_heads == self.self_attn.num_key_value_heads:
            ...
            for w in [self.self_attn.q_proj, self.self_attn.k_proj, self.self_attn.v_proj]:
                ...
                w.weight.data = weight[:-prune_head_num*self.self_attn.head_dim]
        else: # GQA
