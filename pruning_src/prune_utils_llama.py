...
class Pruner:
    ...
    # input weight error compensation
    def adjust_preceding_weight(self,ffn_residual,ori_ffn_residual,preceding_modules=None,percdamp=0.01):

        H = self.p_H / 2
        dead = torch.diag(H) == 0
        H[dead, dead] = 1
        delta_H = self.p_delta_H / 2
        ...
        if preceding_modules is None:
            preceding_modules = [self.module]
        for module in preceding_modules:
            W = module.weight.data.float().t()
            # [hidden_dim(in),hidden_dim] [hidden_dim,hidden_dim] [in,out]
            delta_W = Hinv @ delta_H @ W
            assert not torch.isnan(delta_W).any(), f"preceding weight adjust contains NaN values"
            assert not torch.isinf(delta_W).any(), f"preceding weight adjust contains inf values"
            W = W + delta_W
            # module.weight.data = W.t().to(torch.float16)
        ...
        for i in range(len(ffn_residual)):
            inp = ffn_residual[i].to(self.dev)
            ori_inp = ori_ffn_residual[i].to(self.dev)
            if len(inp.shape)==3:
                inp = inp.squeeze(0)
                ori_inp = ori_inp.squeeze(0)
            inp = inp.t().float()
            ori_inp = ori_inp.t().float()
            delta_inp = ori_inp - inp  # [in,seq]
            if self.p_H is None:
                self.columns = inp.shape[0]  # hidden_dim
                self.p_H = torch.zeros((self.columns, self.columns), device=self.dev).float()
                self.p_delta_H = torch.zeros((self.columns, self.columns), device=self.dev).float()

            self.p_H *= self.p_nsamples / (self.p_nsamples + 1)
            self.p_delta_H *= self.p_nsamples / (self.p_nsamples + 1)
            self.p_nsamples += 1
            self.p_H += inp.matmul(inp.t()) * 2 / self.p_nsamples  # [hidden_dim,seq_len] [seq_len,hidden_dim]
            self.p_delta_H += inp.matmul(delta_inp.t()) * 2 / self.p_nsamples
        ...
        residual_delta_W = Hinv @ delta_H
        assert not torch.isnan(residual_delta_W).any(), f"residual adjust contains NaN values"
        assert not torch.isinf(residual_delta_W).any(), f"residual adjust contains inf values"
        self.ffn_residual_map = (torch.eye(self.columns).float().to(self.dev) + residual_delta_W).to(torch.float16)
    ...
    # local greedy refinement plus weight reconstruction
    def get_mask_weight_adjust(self,sparsity,num_consecutive=1,last_time_mask=None,percdamp=0.01,ffn_inps=None,ori_ffn_inps=None):
        ...
        last_time_mask = last_time_mask.cpu()

        # change to mask of each column
        if num_consecutive!=1:
            copy_last_mask = last_time_mask
            last_time_mask = []
            for i in range(copy_last_mask.shape[0]):
                last_time_mask.extend([True]*num_consecutive if copy_last_mask[i].item()==True else [False]*num_consecutive)
            last_time_mask = torch.tensor(last_time_mask)

        W = W[:,last_time_mask]
        self.rows = W.shape[0]
        self.columns = W.shape[1]
        H = self.H[:,last_time_mask][last_time_mask,:]
        dead = torch.diag(H) == 0
        H[dead, dead] = 1
        H = H.double()
        W[:, dead] = 0
        W = W.double()  # a new deep copy of W
        ...
        while True:
            try:
                H_tmp = torch.linalg.cholesky(H)
                if torch.isnan(H_tmp).any() or torch.isinf(H_tmp).any():
                    raise RuntimeError("H contains NaN")
                else:
                    H = H_tmp
                    break
            except RuntimeError:
        ...
        while True:
            try:
                H_tmp = torch.linalg.cholesky(H, upper=True)
                if torch.isnan(H_tmp).any() or torch.isinf(H_tmp).any():
                    raise RuntimeError("H contains NaN")
                else:
                    H = H_tmp
                ...
                H[diag, diag] += damp*0.1

        Hinv = H  # upper triangular matrix

        mask = torch.ones(count).cpu().bool()  # 保留的为True,剪掉为False
        rangecount = torch.arange(count).cpu()
        rangecolumns = torch.arange(self.columns).cpu()
        
        if size == 1:
            for dropped in range(num_dropped):
                diag = torch.diagonal(Hinv1)
                scores = torch.sum(W**2,0) / diag
                scores[~mask] = float('inf')
                j = torch.argmin(scores)
                row = Hinv1[j,:]
                d = diag[j]
                update = ((W[:,j]/d).unsqueeze(1)).matmul(row.unsqueeze(0))
                assert not torch.isnan(update).any(),'update nan bug!!!!'
                W -= update
                mask[j]=False
                W[:,~mask]=0
                Hinv1 -= row.unsqueeze(1).matmul(row.unsqueeze(0))/d
        else:
            mask1 = torch.ones(self.columns).bool()

            for dropped in range(num_dropped):
                # count: number of heads
                # size: per head size
                blocks = Hinv1.reshape(count, size, count, size)  # [count,size,count,size]
                blocks = blocks[rangecount, :, rangecount, :]  # [count,size,size]
                try:
                    invblocks = torch.cholesky_inverse(torch.linalg.cholesky(blocks))
                except:
                    print('head use pseudo inverse which might cause problem')
                    invblocks = torch.linalg.pinv(blocks, hermitian=True)

                W1 = W.reshape((self.rows, count, size)).transpose(0, 1)  # [count,rows,size]
                W_Hinv_SS_inv = W1 @ invblocks
                scores = (W_Hinv_SS_inv @ W1.transpose(-1,-2)).diagonal(dim1=-1,dim2=-2).sum(dim=-1)

                scores[~mask] = float('inf')
                j = torch.argmin(scores)

                rows = Hinv[(size * j):(size * (j + 1)), :]  # 这行代码应该有问题，当时写错了..., 换成下面那行才对
                #rows = Hinv1[(size * j):(size * (j + 1)), :]

                update = W_Hinv_SS_inv[j].matmul(rows)

                assert not torch.isnan(update).any(), f"{self.module_name},update weight contains NaN values"
                W -= update
                mask[j] = False
                mask1[(size * j):(size * (j + 1))] = False
                W[:, ~mask1] = 0

                for k in range((size * j), (size * (j + 1))):
                    diag = torch.diagonal(Hinv1)
                    d = diag[k]
                    row = Hinv1[k,:]
                    Hinv1 -= row.unsqueeze(1).matmul(row.unsqueeze(0)) / d
                Hinv1[rangecolumns[~mask1], rangecolumns[~mask1]] = 1
        ...
        # rearrange
        indices_zero = torch.where(searched_mask == False)[0]
        indices_one = torch.where(searched_mask == True)[0]
        new_order = torch.cat((indices_zero, indices_one))
        W = self.module.weight.data
        W = W[:, new_order]
        W = W.double()
        ...
        if size == 1:
            for i in range(num_dropped):
                row = Hinv[i, :]
                d = diag[i]
                update = ((W[:, i] / d).unsqueeze(1)).matmul(row.unsqueeze(0))
                assert not torch.isnan(update).any(), f"{self.module_name},{dropped},update weight contains NaN values"
                W -= update
                W[:, i] = 0
        else:
            for j in range(num_dropped):
        ...
        restore_order = torch.empty_like(new_order)
        for i, idx in enumerate(new_order):
            restore_order[idx] = i
        W = W[:, restore_order]

        self.module.weight.data[:,last_time_mask] = W.to(dtype=dtype)

        if size !=1:
            tmp = copy_last_mask.clone()
            tmp[copy_last_mask] = mask
            self.head_mask = tmp
            tmp = last_time_mask.clone()
            tmp[last_time_mask] = mask1
            self.mask = tmp
        else:
            tmp = last_time_mask.clone()
            tmp[last_time_mask] = mask
            self.mask = tmp
    ...
    def get_mask_weight_adjust_lm(self,sparsity):
        ...
        count = self.layer.mlp.intermediate_size  # number of  neurons
        num_dropped = round(count * sparsity)
    ...
    def get_mask_weight_adjust_iterpca_lm0(self,sparsity):
        ...
        keep_rate = keep_num/self.layer.mlp.intermediate_size
    ...
    def get_mask_weight_adjust_iterpca_lm(self,sparsity):
        ...
        W_gate_up =  self.layer.mlp.gate_up_proj

        dtype = W_gate_up.weight.dtype
        W_ = W_gate_up.weight.to(device=dev, dtype=torch.float64)[:self.layer.mlp.intermediate_size] # gate
        Qb = Qb.to(device=dev, dtype=torch.float64)
        W_gate_up.weight.data[:self.layer.mlp.intermediate_size] = torch.matmul(Qb.T, W_).to(device=dev, dtype=dtype)

        W_ = W_gate_up.weight.to(device=dev, dtype=torch.float64)[self.layer.mlp.intermediate_size:] # up
        Qa = Qa.to(device=dev, dtype=torch.float64)
        W_gate_up.weight.data[self.layer.mlp.intermediate_size:] = torch.matmul(Qa.T, W_).to(device=dev, dtype=dtype)
