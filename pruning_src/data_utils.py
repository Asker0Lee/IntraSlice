...
def get_wikitext2(nsamples, seed, seqlen, model, train_valid_test):
    from datasets import load_dataset

    if 'train' in train_valid_test or 'valid' in train_valid_test:
        traindata = load_dataset(f'wikitext', 'wikitext-2-raw-v1', split='train')
    elif 'test' in train_valid_test:
        testdata = load_dataset(f'wikitext', 'wikitext-2-raw-v1', split='test')
    ...
    if 'train' in train_valid_test:
        ...
        for _ in range(nsamples):
            ...
            # trainloader.append((inp, 1))
        return trainloader

    elif 'valid' in train_valid_test:
        trainenc = tokenizer("\n\n".join(traindata['text']), return_tensors='pt')
        import random
...
def get_c4(nsamples, seed, seqlen, model, train_valid_test):
    from datasets import load_dataset
...
def get_loaders(name, nsamples=128, seed=0, seqlen=2048, model='', train_valid_test='train'):
    if 'wikitext2' in name:
        return get_wikitext2(nsamples, seed, seqlen, model, train_valid_test)
    if 'c4' in name:
        return get_c4(nsamples, seed, seqlen, model, train_valid_test)



############## slicegpt dataset code ##################3
...
def get_dataset(name: str) -> datasets.DatasetDict:
    ...
    if 'c4' in name:
        ds['test'] = ds['validation'][:1100]  # same with DISP-LLM
